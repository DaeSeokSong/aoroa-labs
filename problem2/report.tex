...
\midrule
C2 & \textbf{탐색형 고객} & 65명 & 13.0\% &
  avg\_app\_time 최고(30분), avg\_amount 최고(약 10만원), total\_amount 최고 \\
...
Validation MAE(Prophet 8.26\%, XGBoost 7.36\%)는 held-out 구간의 실제 일반화 성능을
반영하여 공정한 가중치 산출이 가능하다.
\end{warnbox}
...
\begin{tabular}{lcccc}
\toprule
\textbf{모델} & \textbf{Val MAPE} & \textbf{Test MAPE (\%)} & \textbf{RMSE (원)} & \textbf{MAE (원)} \\
\midrule
